...
%     axis([-4 6 -1 7])
 ...
 %% Draw the map relative to the robot   
    % Actual transformation of world (w) with respect to robot (r)
    xt = xr(1,t);
    yt = xr(2,t);
    theta = xr(3,t);
    Rr_w = [    cos(theta) -sin(theta) ;
                sin(theta) cos(theta) ];
    tr_w = [ xt yt ]';
    Tw_r = [    Rr_w' -Rr_w'*tr_w ;
                0 0 1 ];
 
    % transform the map and robot trajectory into the robot frame        
    map_r = Tw_r*[ map ; ones(1,M) ];
    trajectory_r = Tw_r*[ xr(1:2,1:t) ; ones(1,t) ];
    
    % Estimated transformation of world (w) with respect to robot (r)
    xt = mu_S(1,t);
    yt = mu_S(2,t);
    theta = mu_S(3,t);
    Rr_w = [    cos(theta) -sin(theta) ;
    ...
    estimated_map_r = Tw_r*[ reshape( mu_S(4:end,t), 2, [] ) ; ones(1,M) ];
    estimated_trajectory_r = Tw_r*[ mu_S(1:2,1:t) ; ones(1,t) ];    
    
    figure(2); clf; hold on;
    plot(map_r(1,:),map_r(2,:),'go', 'MarkerSize',10,'LineWidth',2);
%     plot(map_r(1,:),map_r(2,:),'m.');
    plot(trajectory_r(1,t),trajectory_r(2,t), 'ro--')
    plot([0 1],[0 0], 'r-')
    plot(estimated_trajectory_r(1,t),estimated_trajectory_r(2,t), 'bx--')
    plot([0 0.75],[0 0], 'b-')
    mu_pos = [0 0];
    S_pos = Rw_r*[S(1,1) S(1,2); S(2,1) S(2,2)]*Rr_w;
    error_ellipse(S_pos,mu_pos,0.75);
    error_ellipse(S_pos,mu_pos,0.95);
    ...
    for i=1:M
          if (~newfeature(i))
              ...
              fj = 2*i;
              plot([0 y(fi,t)*cos(y(fj,t))], [0 y(fi,t)*sin(y(fj,t))], 'c');
              plot(estimated_map_r(1,i),estimated_map_r(2,i), 'bx')
              mu_pos = estimated_map_r(1:2,i);
              S_pos = Rw_r*[S(3+fi,3+fi) S(3+fi,3+fj); S(3+fj,3+fi) S(3+fj,3+fj)]*Rr_w;
              error_ellipse(S_pos,mu_pos,0.75);
          end
    end
    axis equal
%     axis([-4 6 -1 7])
    title('Estimate Relative to Robot Frame')
end
