...
for t=2:length(T)
    ...
    for i=1:I
        ...
        Xp(:,i) = Ad*X(:,i) + ep;
        w(i) = normpdf(y(:,t),sqrt(Xp(1,i)^2 + Xp(3,i)^2),sqrt(Q));
    end
    W = cumsum(w);
    for j=1:I
         seed = W(end)*rand(1);
...
% Set up VideoWriter
vid = VideoWriter('ekfukfparticle.mp4','MPEG-4');
vid.Quality = 100;
vid.FrameRate = 10;
open(vid);

%Record Video
for k = 1:2*length(F)-2
    writeVideo(vid,F(floor(k/2)+1));
end
close(vid);

winopen('ekfukfparticle.mp4')